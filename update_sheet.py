...
# 2. NSE Data Fetcher (VOLUME LOGIC)
        ...
        if response.status_code == 200:
            with zipfile.ZipFile(io.BytesIO(response.content)) as z:
                ...
                with z.open(csv_filename) as f:
                    ...
                    # वॉल्यूम के लिए कॉलम
                    vol_col = next((c for c in ['TtlTradgVol', 'TOTTRDQTY', 'VOLUME'] if c in df.columns), None)
                    
                    if not all([sym_col, close_col, vol_col]):
                        return None
...
if data_to_insert:
    try:
        ...
        print(f"SUCCESS: Volume Sheet Updated successfully for {fetched_date_str}!")
    except Exception as e:
...
2️⃣ टर्नओवर (Turnover/Value) आधारित टॉप 250 शेयर छांटने का कोड:
(यह कोड शेयरों को उनमें लगे कुल पैसे / Value के आधार पर छांटता है, जिससे पेनी स्टॉक्स लिस्ट से बाहर हो जाते हैं)
import gspread
from oauth2client.service_account import ServiceAccountCredentials
import pandas as pd
import requests
